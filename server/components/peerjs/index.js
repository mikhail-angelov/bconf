...
var PeerJs = function () {
  function PeerJs(bus, session, User) {
    var _this = this;
    ...
    this.eventBus.on(this.eventBus.SOCKET_CLOSED, function (client) {
      return _this.onClose(client);
    });
    this.eventBus.on(this.eventBus.SOCKET_ERROR, function (client, err) {
      return _this.onError(client, err);
    });
    this.eventBus.on(this.eventBus.SOCKET_MESSAGE, function (client, data) {
      return _this.onMessage(client, data);
    });
    this.eventBus.on(this.eventBus.NEW_CONNECTION, function (client) {
      return _this.onNewConnection(client);
    });
  }

  _createClass(PeerJs, [{
    key: 'onNewConnection',
    value: function onNewConnection(client) {
    ...
    key: 'onMessage',
    value: function onMessage(client, data) {
      try {
        console.log(data);
