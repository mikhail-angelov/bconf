...
function isAuthenticated() {
  ...
  .use(function (req, res, next) {
    ...
    if (req.user.role === 'guest') {
      ...
      _user2.default.findByIdAsync(req.user._id).then(function (user) {
        if (!user) {
...
function hasRole(roleRequired) {
  ...
  return (0, _composableMiddleware2.default)().use(isAuthenticated()).use(function meetsRequirements(req, res, next) {
    if (_index2.default.userRoles.indexOf(req.user.role) >= _index2.default.userRoles.indexOf(roleRequired)) {
      next();
    } else {
...
function signToken(id, role) {
  return _jsonwebtoken2.default.sign({ _id: id, role: role }, _index2.default.secrets.session, {
    expiresIn: '2d'
  });
}
