...
describe('auth', () => {
  ...
  it('should check valid token', async () => {
    const token =
      'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJleHAiOjE1Mzc2NTYxNDIsIl9pZCI6IjViYTY1MzJjNDNjNTI4YTI4M2E4NmY1NCIsIm5hbWUiOiJ0ZXN0MSIsImVtYWlsIjoidGVzdDFAdGVzdC5jb20iLCJpYXQiOjE1Mzc2MjczNDJ9.z63m_Yu89FcpGdzTe9PRUxkSq0iN-jZMqzS1sdX_FUE'
  ...
  it('should update user settigs', async () => {
    ...
    const response = await auth.updateUser(_id, {
      name: 'kkk23',
      email: 'Kkk@kkk.kkk23',
      srcAvatar: 'user ava src!',
    })
  ...
  it('should create user and login via provider', async () => {
    const { user, token } = await auth.loginViaProvider({
      displayName: 'fb',
      email: 'fb@test.com',
      uid: 'any',
      providerId: 'firebase',
