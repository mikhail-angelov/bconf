...
function messages(state = {
    ...
    switch (action.type) {
        case actions.message.ADD_MESSAGE: {
            ...
            state.chats[state.active] = [message].concat(state.chats[state.active]);
            return state
        }
        ...
        case actions.message.SEARCH_MESSAGE: {
            ...
            if (action.messageText != '') {
                filtred = _.filter(state.chats[state.active], item => {
                    return item.text.indexOf(text) >= 0;
                })
            } else {
                filtred = state.chats[state.active]
            }
        ...
        case actions.message.SET_ACTIVE: {
            return Object.assign(state, {
                ...
                filtered: state.chats[state.active]
            })
        }
